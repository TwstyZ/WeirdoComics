...
<div class="d-flex justify-content-end"><a href="{{route('item.create')}}" class="btn btn-warning mt-1 mx-5 text-white p-2">Add Item</a></div>
    ...
    <div class="d-flex justify-content-center m-3 flex-wrap">
        ...
        @if($consulta->Type!='Comic')
            ...
            <div class="card bg-light m-3 rounded-lg border-gray border-2" style="width: 18rem;">
                ...
                <div class="card-body">
                    <a href="{{ route('item.edit', $consulta->Id_item) }}" class="btn" style="color:white;background-color:#033E8C">Edit</a>
                    {{-- <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#deleteModal{{ $consulta->Id_book }}">Delete</button> --}}
                    <button type="button" class="btn" style="background-color:#D90404;color:white" data-bs-toggle="modal" data-bs-target="#itemDelete{{$consulta->Id_item}}">Delete</button>
                </div>
            </div>
            @endif
            @endforeach
